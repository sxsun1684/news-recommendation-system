...
# Maximum concurrent tasks to control resource usage
MAX_CONCURRENT_TASKS = 10
semaphore = asyncio.Semaphore(MAX_CONCURRENT_TASKS)

async def fetch_bbc_article_datetime(url, browser):
    """Fetches the publication date/time of a single BBC article."""
    async with semaphore:  # Controls concurrency to prevent overload
        page = await browser.new_page()

        # Disable images, CSS, and JS for faster loading
        await page.route("**/*.{png,jpg,jpeg,gif,svg,css,js}", lambda route: route.abort())

        try:
            # Navigate to URL, timeout after 8 seconds
            await page.goto(url, wait_until="domcontentloaded", timeout=8000)
            # Wait up to 3 seconds for the "time" element to appear
            await page.wait_for_selector("time", timeout=3000)
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Concurrently execute fetching tasks
        tasks = [fetch_bbc_article_datetime(url, browser) for url in urls]
        results = await asyncio.gather(*tasks, return_exceptions=True)  # Handle exceptions without interrupting execution
